...
def set_rotation_for_single_bone(target_armature_name, bone_name, vector):    
    # open bone class
    try:
        bone = Bone(bone_name=bone_name, armature_name=target_armature_name)
...
def MP_set_rotation_for_all_bones(armature_selector, mediapipe_output):
    ...
    set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.spine_1, vector=rotation_list[MP_ROOT_ROTATION])
    set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.spine_2, vector=rotation_list[MP_ROOT_ROTATION])
    
    # rotate shoudler
    if mediapipe_output[MP_SHOULDER_L][PRESENCE] > 0.5 and armature_selector.shoulder_L:
        bone = Bone(bone_name=armature_selector.shoulder_L, armature_name=armature_selector.target_armature.name)
        if rotation_list[MP_SHOULDER_L_ROTATION][Z] > 0:
            bone.set_bone_rotation_around_world_space_vector([1, 0, 0], ease_in_out_lerp(0, 45, rotation_list[MP_SHOULDER_L_ROTATION][Z]))
        elif rotation_list[MP_SHOULDER_L_ROTATION][Z] > -0.2:
    ...
    if mediapipe_output[MP_SHOULDER_R][PRESENCE] > 0.5 and armature_selector.shoulder_R:
        bone = Bone(bone_name=armature_selector.shoulder_R, armature_name=armature_selector.target_armature.name)
        if rotation_list[MP_SHOULDER_R_ROTATION][Z] > 0:
            bone.set_bone_rotation_around_world_space_vector([1, 0, 0], ease_in_out_lerp(0, 45, rotation_list[MP_SHOULDER_R_ROTATION][Z]))
        elif rotation_list[MP_SHOULDER_R_ROTATION][Z] > -0.2:
            bone.set_bone_rotation_around_world_space_vector([1, 0, 0], ease_in_out_lerp(0, -30, rotation_list[MP_SHOULDER_R_ROTATION][Z] / 0.2))
        else:
            bone.set_bone_rotation_around_world_space_vector([1, 0, 0], -30)
    
    # align limbs to world vector
    if mediapipe_output[MP_SHOULDER_L][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.upperarm_L, vector=rotation_list[MP_SHOULDER_L_ROTATION])

    if mediapipe_output[MP_ELBOW_L][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.forearm_L, vector=rotation_list[MP_ELBOW_L_ROTATION])

    if mediapipe_output[MP_HAND_L][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.hand_L, vector=rotation_list[MP_HAND_L_ROTATION])

    if mediapipe_output[MP_HIPS_L][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.thigh_L, vector=rotation_list[MP_HIPS_L_ROTATION])

    if mediapipe_output[MP_KNEES_L][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.shin_L, vector=rotation_list[MP_KNEE_L_ROTATION])

    if mediapipe_output[MP_TOE_L][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.foot_L, vector=rotation_list[MP_FOOT_L_ROTATION])

    if mediapipe_output[MP_SHOULDER_R][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.upperarm_R, vector=rotation_list[MP_SHOULDER_R_ROTATION])

    if mediapipe_output[MP_ELBOW_R][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.forearm_R, vector=rotation_list[MP_ELBOW_R_ROTATION])

    if mediapipe_output[MP_HAND_R][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.hand_R, vector=rotation_list[MP_HAND_R_ROTATION])

    if mediapipe_output[MP_HIPS_R][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.thigh_R, vector=rotation_list[MP_HIPS_R_ROTATION])

    if mediapipe_output[MP_KNEES_R][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.shin_R, vector=rotation_list[MP_KNEE_R_ROTATION])

    if mediapipe_output[MP_TOE_R][PRESENCE] > 0.5:
        set_rotation_for_single_bone(target_armature_name=armature_selector.target_armature.name, bone_name=armature_selector.foot_R, vector=rotation_list[MP_FOOT_R_ROTATION])
